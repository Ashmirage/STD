...
{
    HAL_TIM_IC_Start_IT(&htim1, REMOTE_IN_TIMX_CHY);    /* 开始捕获TIM的通道值 */
...
void Remote_IC_CaptureCallback(void)
{
	...
	if (RDATA)      /* 上升沿捕获 */
	{
		__HAL_TIM_SET_CAPTUREPOLARITY(&htim1, REMOTE_IN_TIMX_CHY, TIM_INPUTCHANNELPOLARITY_FALLING);    /* CC4P=1 设置为下降沿捕获 */
		__HAL_TIM_SET_COUNTER(&htim1, 0);       /* 清空定时器值 */
		g_remote_sta |= 0x10;                           /* 标记上升沿已经被捕获 */
	}
	else           /* 下降沿捕获 */
	{
		dval=HAL_TIM_ReadCapturedValue(&htim1, REMOTE_IN_TIMX_CHY);         /* 读取CCR1也可以清CC1IF标志位 */
		__HAL_TIM_SET_CAPTUREPOLARITY(&htim1, REMOTE_IN_TIMX_CHY, TIM_INPUTCHANNELPOLARITY_RISING);     /* 配置TIM4通道4上升沿捕获 */
