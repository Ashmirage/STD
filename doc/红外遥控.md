...
- 当前代码固定是 `PA8 = TIM1_CH1`
- 如果你只是换成同一个定时器同一个通道的别的复用脚，主要改这些宏：
  - `REMOTE_GPIO_PORT`
  - `REMOTE_GPIO_PIN`
  - `REMOTE_GPIO_PINSRC`
  - `REMOTE_GPIO_AF`
- 如果 GPIO 端口变了，`remote_init()` 里 `RCC_AHB1PeriphClockCmd(...)` 也要跟着改
- 如果你换了定时器，不只是改宏，还要一起改这些地方：
  - `REMOTE_TIM`
  - `REMOTE_TIM_CHANNEL`
  ...
  - `TIM_IT_CC1`
  - `TIM_GetCapture1(...)`
  - `remote_set_edge()` 里操作的 `CC1` 相关位
- 如果新定时器挂在 `APB1`，还要把 `RCC_APB2PeriphClockCmd(...)` 改成对应的 `RCC_APB1PeriphClockCmd(...)`
- 如果你的遥控器地址码不是 `0`，还要改 `driver/bsp/remote.h` 里的：
